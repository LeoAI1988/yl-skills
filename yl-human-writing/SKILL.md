---
name: yl-human-writing
description: 通用中文创作与改稿 Skill。用于知乎回答、论坛长帖、公众号文章、博客、评论、人物故事、历史叙事、新闻与行业解读、科普、教程、评测、个人叙事、小说、故事、对白、口播和演讲稿。默认写成一个见过事、查过材料、愿意把来龙去脉讲清楚的人在说话，重点保留中文互联网长回答与长帖的活人感和自然中文韵律，避免空泛的机构腔、喊口号式演说腔、营销腔和模型腔。非虚构长文先检查材料够不够，材料不足时研究、追问或缩短，绝不用重复解释灌字数。现实内容额外核验事实、引语、数据与用户亲历，虚构内容可以创造人物、场景、对白、心理与情节。默认清理公式化翻案、滥用冒号和破折号、商业黑话与模型惯用黑话；用户亲手修改或确认的自然表达优先于这些默认禁令。不创建新的个人写作 Skill。
---
...
公开适配版 1.1.0，基于 upstream human-writing 1.1.0。完整保留通用中文方法、现实/虚构分流和检查器，原始 MIT 许可与署名见 [LICENSE](LICENSE)，来源说明见 [SOURCE.md](SOURCE.md)。这是独立的通用写作引擎，不是卡兹克版本；选择本引擎时不叠加其他风格的全部句式禁令。

使用要求：Python 3.10 或更新版本，脚本只用标准库，不依赖任何私人 Skill、账号或凭证。素材、草稿、作者校准与检查报告均放在安装者自己的工作区。先尊重安装者已经确认的原话和文体；下述默认禁令不能覆盖本轮明确保留的表达。
...
当作品以当前用户身份发表，或用户交回亲手修改的版本时，先与上一版逐处对照，学习每一处修改承担的功能。不要把明显笔误、未核实事实或单篇题材词汇当成稳定文风。

- 平台转换只迁移传播机制，不机械搬运形式。短视频的高能量判断可以保留，公众号要提升措辞和铺陈，让判断先立住，再迅速进入事件。
- 不写“我昨天发过一条视频”“上一篇文章说过”“播放量多少”等幕后元叙述，除非这些数据本身就是事件证据。
- 科技事件优先按“判断与事件 → 新东西是什么 → 与主流有什么不同 → 第一性原理 → 人话解释 → 动作背后的意义”推进。顺序服务理解，不把它写成六个机械小标题。
- 技术只解释到普通人能理解原理和差异的位置。解释清楚就停，把篇幅留给组织方式、力量对比和这件事将把局面推向哪里。
- 历史、商业案例和典故融入同一条因果线，点到为止。不要逐项与当前事件一一对标，也不要把读者已经能体会的隐喻再解释干。
- 不替读者命名情绪，不写“让人后背发凉”“这是某某脊梁”一类硬总结。让事实、次序和留白把读者带到那里。
- 允许自然问句、轻微调侃、短句停顿和有意的对照。不要把它们加工成固定口号或每段一个金句。
- 公众号观点、趋势、产品和营销文章默认控制在 1800 个中文非空白字符以内，2000 为硬上限；用户明确要求长篇调查、口述史或其他体裁时除外。
- 少分节。只有信息类型真正切换，例如从观点进入安装教程，才使用小标题。

微信公众号结尾使用安装者本轮提供或确认的版本。没有配置时自然收束，不继承任何个人签名、赠品导流或联系方式。

用户确认正文后，把该 Markdown 当作唯一正文源。排版、HTML、短视频或其他派生物不得顺手纠错、润色或恢复旧结尾。
...
- 模型新写的引语也需检查；安装者明确要求保留的真实原话优先。其余原话若需改写，先保证事实与语气不变，不能为清零而删掉关键材料。

“不只……还……”是正常中文的递进，可以用。它出现在翻案位置、用来给下文抬价时，按翻案腔处理。

初稿写完以后再执行这些检查。修改违禁句时，先找它原本想说的事实，随后用普通句子说出来。不要寻找另一种漂亮句式替换。

长稿可以保存为 Markdown 或文本后运行 `scripts/check_prose.py <稿件路径>`。脚本失败时回到正文人工判断；修改模型新写的违规表达，安装者明确要求保留的真实原话或特定文体记录为有理由的例外，不为清零破坏正文。
