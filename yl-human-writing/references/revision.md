公共版适配说明：以下来源中的绝对禁令受同目录 `writing-foundation.md` 的原话、文体与语境规则约束。
...
模型新写的表达按本文件检查；安装者明确要求保留的真实原话和文体要求优先。脚本无法识别这种授权，遇到命中时人工判断并记录例外，不因检查器建议改掉已经批准的原话。
...
公共版脚本把冒号、破折号、对照句、词表和模型路标列为待语义判断候选，不能按返回码直接判为写作失败。它会把需要结合语境判断的词列为提醒，还会提醒短句排队、段落过度单一、固定开场与多套比喻。

脚本只负责发现形状，不能判断文章有没有人。警告项需要回到材料与说话位置人工判断。诗歌、剧本和实验写作同样把脚本结果作为候选，散文形状警告服从具体文体。
