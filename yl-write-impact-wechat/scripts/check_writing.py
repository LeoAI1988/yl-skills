#!/usr/bin/env python3
"""Human Writing-derived heuristic candidate scan. No authorship or quality score; read-only."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="初筛写作候选，必须结合公共规则逐项语义裁决")
    ...
    for position, phrase in stop_matches:
        failures.append(f"来源硬停词候选，第 {line_number(text, position)} 行，{phrase}")

    jargon_matches = non_overlapping_terms(prose, HARD_JARGON)
    for position, phrase in jargon_matches:
        failures.append(f"词语语境候选，第 {line_number(text, position)} 行，{phrase}")
    ...
    for match in pivots:
        failures.append(
            f"对照/翻案候选，第 {line_number(text, match.start())} 行，"
    ...
    for match in anaphoras[:4]:
        warnings.append(
            ...
            f"“{excerpt(match.group(), 44)}”。核对每项是否递进，功能性排比保留，不按数量删原话。"
        )
    ...
    if len(highlights) > highlight_limit:
        ...
        warnings.append(
            f"「」括起的短语共 {len(highlights)} 处。{samples}。密集时核对是否有实际表达作用。"
        )
    ...
    report = {
        ...
        "hash_scope": "UTF-8 text after decoding; original line endings preserved; BOM excluded",
        "han_count": total_han,
        "candidate_count": len(candidates),
        "candidates": [{"id": "C%03d" % (i + 1), "detail": item,
                        "decision": "unresolved"} for i, item in enumerate(candidates)],
        "limitations": "Heuristics only. Zero hits is not approval. Read H-01..H-12, D-01..D-22 and E-01..E-05; resolve each candidate against sources, genre and function. No AI probability.",
        "exit_codes": {"0": "no mechanical candidates; semantic review still required",
                       "1": "candidates need review", "2": "input cannot be checked"}
    }
